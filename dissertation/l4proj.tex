...
\begin{enumerate}
    \item As time increments, the first action carried out by the power domain is to execute any defined events allocated by the user \emph{(see section \ref{sec:events}}), this ensures that these actions change the state of the simulation before the normal workflow occurs.
...
As a result of Extended LEAF allowing for power consumption outside the power domain's workflow, the framework also provides a means to allow for these actions to be logged, ensuring that the file results of a simulation reflect the events that occurred.

\section{Power Sources}

The responsibility of a power source in Extended LEAF is to provide power to entities in the infrastructure it is associated to.
This is because, as LEAF was originally based around the idea of infrastructure consuming power through both static and dynamic means \citep{leaf2021}, power sources act in the opposite manner and see to generate it.
As a result of this, Extended LEAF expects power sources to be defined by the user in the unit of watts and assumes that the rate of power transmission is instantaneous as the latency of transmission is negligible \citep{speed-of-electricity}.
This was done to ensure that when retrieving a infrastructure's power measurement, direct comparisons could be made without the need of transforming the data into another unit of measurement .
As a result of Extended LEAF operating in discrete events of time ($\delta T$) power in reality once measured is energy provided by the power source and consumed by the entity.\\

As previously mentioned, power sources interact with the infrastructure through the power domain and receive infrastructure through the distribution mechanism employed by the power domain.
Because of this, when a power source is defined by the user as dynamic due to the variable entities present during the simulation.
However, to allow for scenarios where distributing infrastructure between power sources is redundant (for instances a set of entities that will always reside with a particular power source), Extended LEAF also allows for a power source to be defined as static.
...
For instance when considering entities in the infrastructure that are mobile, providing power from a battery is a necessity but is essential that these entities do not receive power from another source in the power domain as in reality there would be no cable supplying power to the entity.\\
...
This is a result of the fact that power sources need to be characterised into groups based on attributes inherent to them.
