...
from src.extendedLeaf.power import PowerMeter, GridPower, SolarPower, PowerDomain
...
def main():
    ...
    power_domain = PowerDomain(env, name="Power Domain1", associated_nodes=local_site.infrastructure.nodes(), start_time_str="11:00:00",
                               update_interval=1)
    solar_power1 = SolarPower(env, name="solar1", power_domain=power_domain,
                             data_set_filename="08-08-2020 Glasgow pv data.csv", priority=0)
    solar_power2 = SolarPower(env, name="solar2", power_domain=power_domain,
                             data_set_filename="08-08-2020 Glasgow pv data.csv", priority=1)
    solar_power3 = SolarPower(env, name="solar3", power_domain=power_domain,
